...
import random
import util
from classes.base import DungeonTile, DungeonRoom, DungeonObject, Item
from classes.characters import Fighter, BasicMonster


# Takes a level to configure dungeon settings
        ...
        for i in range(num_monsters):
            ...
            if not util.is_blocked(x, y, self):
                if random.randint(0, 100) < 80:  # 80% chance of getting an orc
                    ...
                    monster = DungeonObject(x, y, 'o', 'orc', fgcolor=(150, 250, 230), blocks=True,
                    ...
                    monster = DungeonObject(x, y, 'T', 'troll', fgcolor=(100, 180, 150), blocks=True,
                                            fighter=fighter_component, ai=ai_component)
    ...
    def place_item(self, rooms):
        for i in range(self.num_items):
            ...
            if not util.is_blocked(x, y, self):
                ...
                if self.level_id == 5:
                    item = DungeonObject(x, y, '!', 'Cone of Dunshire', item=item_component)
                    self.cone = item
                else:
                    item = DungeonObject(x, y, '!', 'health potion', item=item_component)
